...
- `configs/h100-8gpu-autonomous.yaml` uses the throughput layout for a new
  random-initialized, self-play-only run. It rejects imported training
  artifacts, records immutable provenance, caps update-to-data, and mixes only
  checkpoints generated by its own run.
